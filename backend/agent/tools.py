...
def read_sensors(zone_id: str | None = None) -> dict:
    ...
    Args:
        zone_id: Specific zone to read (A-D), or None for all zones.
    """
...
@tool
def treat_disease_uvc(zone_id: str) -> dict:
    """Apply UV-C water sterilization to a zone. Very effective (99% pathogen kill) but costs 2kW power.

    Args:
        zone_id: Zone identifier (A-D)
    """
    return _get_engine().apply_command({
        "action": "treat_disease_uvc",
        "zone_id": zone_id,
    })


@tool
def treat_disease_h2o2(zone_id: str) -> dict:
    """Apply hydrogen peroxide treatment to a zone. Moderately effective (50-75%) and temporarily raises EC.

    Args:
        zone_id: Zone identifier (A-D)
    """
    return _get_engine().apply_command({
        "action": "treat_disease_h2o2",
        "zone_id": zone_id,
    })


@tool
def adjust_humidity(zone_id: str, target_humidity: float) -> dict:
    """Adjust humidity setpoint for a zone. Lower humidity (below 60%) slows fungal diseases.

    Args:
        zone_id: Zone identifier (A-D)
        target_humidity: Target humidity percentage (30-90%)
    """
    return _get_engine().apply_command({
        "action": "adjust_humidity",
        "zone_id": zone_id,
        "params": {"target_humidity": target_humidity},
    })


@tool
def remove_infected_crops(zone_id: str, crop_name: str) -> dict:
    """Remove contaminated crop material from a zone. Sacrifices the crop but reduces disease severity by 30.

    Args:
        zone_id: Zone identifier (A-D)
        crop_name: Name of the crop to remove
    """
    return _get_engine().apply_command({
        "action": "remove_infected_crops",
        "zone_id": zone_id,
        "params": {"crop_name": crop_name},
    })
